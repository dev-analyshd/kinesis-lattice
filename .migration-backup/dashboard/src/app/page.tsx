...
export default function Dashboard() {
        ...
        try {
          ...
          if (msg.type === 'topology') {
            ...
            if (silentAgents.length > 0) {
              const event = `${new Date().toLocaleTimeString()}: ${silentAgents.map(a => a.name).join(', ')} entered silence`;
              setSilenceEvents(prev => [event, ...prev.slice(0, 19)]);
            }
          }
        } catch (err) {
  ...
  // Spawn demo agent button handler
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-[#e2e8f0] font-mono">
      ...
      <div className="grid grid-cols-12 gap-4 p-4">
        ...
        {/* Lattice Graph — force-directed agent topology */}
        <motion.div
        ...
        {/* Silence Feed — full width bottom */}
        <motion.div
